...
$generalFilesLibrary = @(
  $receiptFolders | ForEach-Object {
    ...
    $entries = @(
      Get-ChildItem -Path $folder.FullName -Recurse -File -ErrorAction SilentlyContinue |
        ...
        ForEach-Object {
          [pscustomobject]@{
            name = $_.Name
            relativePath = Get-RelativeWebPath -RootPath $hubRoot -TargetPath $_.FullName
            extension = $_.Extension.ToLowerInvariant()
            sizeBytes = [int64]$_.Length
            sizeLabel = Format-FileSize $_.Length
            parentFolder = Split-Path (Get-RelativeWebPath -RootPath $folder.FullName -TargetPath $_.DirectoryName) -Leaf
          }
        }
    )
    ...
    [pscustomobject]@{
      title = $folder.Name
      relativePath = Get-RelativeWebPath -RootPath $hubRoot -TargetPath $folder.FullName
...
$data = [pscustomobject]@{
  ...
  resourceLibrary = $generalFilesLibrary
}
