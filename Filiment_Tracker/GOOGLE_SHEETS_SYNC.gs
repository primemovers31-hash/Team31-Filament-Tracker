...
function doPost(e) {
  try {
    ...
    if ((data.action || "") === "replaceRows") {
      return handleReplaceRows(data);
    }
...
function handleUpsert(data) {
  try {
    ...
    if (action === "delete") {
      ...
      return jsonResponse({ ok: true, row: rowIndex, deleted: true });
    }
...
function handleReplaceRows(data) {
  try {
    var secret = PropertiesService.getScriptProperties().getProperty("SHARED_SECRET") || "";
    if (secret && data.secret !== secret) {
      return jsonResponse({ ok: false, error: "Unauthorized" });
    ...
    var sheetName = data.sheetName || "BambuLive";
    ...
    if (!headers.length) {
      ...
      headers = Object.keys(discovered);
    }

    sheet.clearContents();

    if (!headers.length) {
      return jsonResponse({ ok: true, rowCount: 0, sheetName: sheetName });
    }

    var matrix = [headers];
    rows.forEach(function(row) {
      matrix.push(headers.map(function(header) {
        var value = row && typeof row === "object" ? row[header] : "";
    ...
    return jsonResponse({ ok: true, rowCount: rows.length, sheetName: sheetName });
  } catch (error) {
    return jsonResponse({ ok: false, error: String(error) });
  }
}
